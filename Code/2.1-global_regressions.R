...
# t = months between eom and set$start
...
# Estimate regression ----------------
estimates <- features |> map(function(feat) {
  print(paste0(feat, ": ", match(feat, features), " of ", length(features)))
  # Data
  sub <- chars[, .(excntry, eom, dev, us, excntry_eom, t, "me_rank"=market_equity, rvol, var = get(feat), ret_exc_lead1m)]
  # Regressions
  reg_list <- list(
    ...
    "liq" = felm(ret_exc_lead1m ~ var*me_rank | excntry_eom | 0 | excntry+eom, data = sub),
